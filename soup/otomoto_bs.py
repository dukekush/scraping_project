...
for link in offers_links:
    ...
    try:
        color = bs.find('span', string ='Kolor').find_next_sibling('div').a.get_text(strip = True, separator =' ')
        color_list.append(color)
    except:
        color_list.append(np.nan)
        
    # try:
    #     driven = bs.find('span', string ='Przebieg').find_next_sibling('div').get_text(strip = True, separator =' ')[:-3].replace(" ", "")
    #     driven_list.append(driven)
    # except:
    #     driven_list.append(np.nan)

    try:
	    make = bs.find('span', string ='Marka pojazdu').find_next_sibling('div').a.get_text(strip = True, separator =' ')
		make_list.append(make)
	except:
		make_list.append(np.nan)

	try:
		model = bs.find('span', string ='Model pojazdu').find_next_sibling('div').a.get_text(strip = True, separator =' ')
		model_list.append(model)
	except:
		model_list.append(np.nan)

	try:
		power = bs.find('span', string ='Moc').find_next_sibling('div').get_text(strip = True, separator =' ').replace(" ", "")
	...
	try:
		year = bs.find('span', string ='Rok produkcji').find_next_sibling('div').get_text(strip = True, separator =' ')
		year_list.append(year)
	except:
		year_list.append(np.nan)

	try:
		fuel = bs.find('span', string ='Rodzaj paliwa').find_next_sibling('div').a.get_text(strip = True, separator =' ')
		fuel_list.append(fuel)
	except:
		fuel_list.append(np.nan)

	# try:
	# 	capacity = bs.find('span', string ='Pojemność skokowa').find_next_sibling('div').get_text(strip = True, separator =' ').replace(" ", "")
    #     capacity_list.append(capacity)
	# except:
	# 	capacity_list.append(np.nan)

	try:
		transmission = bs.find('span', string ='Skrzynia biegów').find_next_sibling('div').a.get_text(strip = True, separator =' ')
		transmission_list.append(transmission)
	except:
		transmission_list.append(np.nan)

	try:
		drive = bs.find('span', string ='Napęd').find_next_sibling('div').a.get_text(strip = True, separator =' ')
...
data = pd.DataFrame({"Make": make_list, "Model": model_list, "Price [PLN]": price_list, "Power [hp]": power_list,
                    ...
                    "Color": color_list, "Year of production": year_list, "Fuel": fuel_list,
					#"Engine capacity [cm^3]": capacity_list,
                    "Transmission type": transmission_list, "Drive type": drive_list})
